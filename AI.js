...
function terminal(board){
	...
	for (row of board){
		for (col of row){
			...
			//is not over/
			if (JSON.stringify(col) == JSON.stringify(" ")){
				return false;
			}
		}
	}
...
function utility(board){
	...
	if (JSON.stringify(winner(board)) == JSON.stringify("X")){
		return 1;
	} else if (JSON.stringify(winner(board)) == JSON.stringify("O")){
		return -1;
	}
...
function minvalue(board){
	...
	for (actionMin of boardActions(board)){
		function temp(minV, actionMin){
			minV = Math.min(minV, maxvalue(result(board, actionMin)));
			return minV;
		}
		minV = temp(minV, actionMin);
	}
	return minV;
}
...
function maxvalue(board){
	...
	for (actionMax of boardActions(board)){
		function temp(maxV, actionMax){
			maxV = Math.max(maxV, minvalue(result(board, actionMax)));
			return maxV;
		}
		maxV = temp(maxV, actionMax);
	}
	return maxV;
}